function plot_end_point_error_contour_corr_sac(data_path)

img_save_path = fullfile(data_path, 'population_figs', 'behavior');

JDM_params_funcs;
S = load(fullfile(data_path, 'population_data', 'sac_kinematic_corr_sac.mat'));

subj_names  = params.animal_list;
num_subjects = numel(subj_names);

group_labels = {'HH,LH', 'HL,LL'};
Line_Color   = [190, 37, 43; 0, 10, 255] / 255;

%% Loop through subjects
for jj = 1:num_subjects
    data_error = S.tag4_end_point_error{jj,1}(1:4);
    % HH, LH = [1;4], HL,LL = [2;3]
    data_groups = {
        [data_error{1}; data_error{4}], ...
        [data_error{2}; data_error{3}]
    };
    ...
    for i = 1:2
        ...
        % --- normalize so grid sums to 1 ---
        f_cell = f_cell / sum(f_cell(:));
        f_cell_all{i} = f_cell;
    ...
    for i = 1:2
     ...
     % ---- Title with only mass levels ----
        level_str = sprintf('Levels: %s', ...
            strjoin(arrayfun(@(l) sprintf('%d%%', round(l*100)), ...
            levels, 'UniformOutput', false), ', '));
        
        title({ ...
            sprintf('Corrective sac. %s - %s', subj_names{jj}, group_labels{i}), ...
            level_str}, ...
            'FontSize', 9);
    ...
    % Save figure
    fname = sprintf('%s-high-vs-low-endpoint-density_corr_sac.pdf', subj_names{jj});
    ESN_Beautify_Plot(fig, [10, 5]);
