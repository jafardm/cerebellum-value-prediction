function JDM_build_CS_rate_dir_prim_sac(data_path)
JDM_params_funcs
...
file_name = 'CS_rate_dir_prim_sac';
...
tags = 1;
win_vis = [-50,150]; % +50 150 ms
win_sac = [50,50]; % -50 to 50 ms
...
sac_data_dir_sac   = MAF_bin_by_ang(neur_data_sac,sac_data_sac,'onset');
 ...
 for counter_dir = 1:n_dirs  
    for cond_idx = 1:n_conds
       ...
       istuned_vis = ismember([task_cond' task_tag' tgt_cond'],...
       ...
       istuned_sac = ismember([task_cond' task_tag' tgt_cond'],...
        force_conds(cond_idx,:),'rows');
...
function neural_sac_data = JDM_combine_dataset_cs(data_recs, rec_info, alignment_vec, time_span, tags_of_interest)
    ...
    for counter_alignment = 1:numel(alignment_vec)
        ...
        for counter_rec = 1:num_rec
            ...
            neural_sac_data_ = MAF_buildSacData(current_rec.eye, Neural_Data, current_alignment, mod_span, tags_of_interest);
