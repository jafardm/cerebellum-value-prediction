function JDM_build_SS_MLI_dataset_all_tags(data_path)
    JDM_params_funcs
        ...
        for counter_sess = 1:num_sess
            ...
            for counter_clust = 1:num_clust
               ...
               if current_clust <= 0, continue; end
                ...
                % Skip this cluster if CS-on directionality is too weak
    %             if mean(cs_on_rho_tot, 'omitnan') < 0.15
    %                 disp('Weak CS-on vector, skipping this clique.');
    %                 continue;
    %             end
    ...
    for counter_var = 1:numel(cell_var)
        ...
        file_name = [current_var '_population_clique_all_sac'];
    ...
    % Save CS
    data = data_out.CS;
    file_name = 'CS_population_clique_all_sac.mat';
...
function res = load_cs_data_tuned(current_path,current_sess,current_cell,...
 ...
 for counter_dir = 1:n_dirs  
       ...
       mask_sac   = has_cs_sac > 0;              % <= same here
