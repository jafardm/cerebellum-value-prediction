function JDM_build_SS_dataset_prim_sac(data_path)
% If Vel_edges is omitted or empty, we will use 3 bins (tertiles) per animal.
    ...
    n_conds = size(forced_conds,1) + size(choice_conds,1);

    % amplitude bins
    Amp_edges = [2 3 4 5 100];
            ...
            for counter_clust = 1:num_clust
                ...
                for counter_var = 1:numel(cell_var)
                    ...
                    for counter_cell = 1:num_cell_clust
                        ...
                        res = load_ss_data_tuned(current_path,current_sess,current_cell, ...
                              order_ang,sac_crit,data_eye,forced_conds,choice_conds,Amp_edges,Vel_edges,tag_id);
                ...
                for counter_cs = 1:num_cs_clust
                    ...
                    res = load_cs_data_tuned(current_path,current_sess,current_cs, ...
                          order_ang,sac_crit,data_eye,forced_conds,choice_conds,tag_id);
    ...
    meta.choice_conds   = choice_conds;
    ...
    for counter_var = 1:numel(cell_var)
        ...
        file_name = [current_var '_population_clique_prim_sac'];
    ...
    % save CS
    data = data_out.CS;
    file_name = 'CS_population_clique_prim_sac.mat';
...
for counter_dir = 1:8
    for cond_idx = 1:n_conds
        ...
        choice_idx = sac_data_dir(order_ang(counter_dir)).sac.choice;
        ...
        if cond_idx <= n_forc_cond
            ...
            istuned_conds = ismember([task_cond' choice_idx'], ...
...
function res = load_cs_data_tuned(current_path,current_sess,current_cell,...
    order_ang,sac_crit,data_eye,force_conds,choice_conds,tag_id)
    n_timepoints = 500;
    n_dirs       = 8;
 ...
 for counter_dir = 1:n_dirs  
    for cond_idx = 1:n_conds
       ...
       if cond_idx <= n_forc_cond 
        ...
        choice_idx = sac_data_dir_sac(order_ang(counter_dir)).sac.choice;
       ...
       if cond_idx <= n_forc_cond     
           ...
           istuned_sac = ismember([task_cond' choice_idx'],...
            choice_conds(cond_idx - n_forc_cond,:),'rows');
       end
    ...
    res.rate_tot = cat(4, cs_rate_tot_vis, cs_rate_tot_sac); % [500 x 8 x 6 x 2]
    res.vm_tot   = cat(4, cs_vm_tot_vis, cs_vm_tot_sac);

end
%% assign_cliques_to_cs
% function cell_cliques = assign_cliques_to_cs_from_nearest_noncs(cell_ids, cell_types, cell_cliques)
% % Assigns the clique of the nearest non-CS cell (within 3 channels)
% % to each CS cell in the input list.
% 
%     % Preserve original cliques
%     cell_cliques_original = cell_cliques;
% 
%     % Find CS cells and their indices
%     CS_cells = cell_ids(ismember(cell_types, 'CS'));
%     cs_inds = find(ismember(cell_types, 'CS'));
% 
%     % Extract channel numbers
%     cs_ch_num = cellfun(@(x) str2double(regexp(x, '(?<=^[^_]+_[^_]+_)[^_]+', ...
%         'match', 'once')), CS_cells);
%     cell_ch_num = cellfun(@(x) str2double(regexp(x, '(?<=^[^_]+_[^_]+_)[^_]+', ...
%         'match', 'once')), cell_ids);
% 
%     % Non-CS pool
%     non_cs_mask = ~ismember(cell_types, 'CS');
%     non_cs_ids = cell_ids(non_cs_mask);
%     non_cs_ch_num = cell_ch_num(non_cs_mask);
% 
%     for i = 1:length(cs_ch_num)
%         % Distance to all non-CS channels
%         diff = abs(non_cs_ch_num - cs_ch_num(i));
% 
%         % Candidates within 3 channels
%         valid_idx = find(diff <= 3);
%         if ~isempty(valid_idx)
%             % Closest one
%             [~, min_rel_idx] = min(diff(valid_idx));
%             nearest_idx = valid_idx(min_rel_idx);
% 
%             % Find full index in original list
%             nearest_non_cs_id = non_cs_ids{nearest_idx};
%             full_idx = find(strcmp(cell_ids, nearest_non_cs_id));
% 
%             if ~isempty(full_idx)
%                 cell_cliques(cs_inds(i)) = cell_cliques_original(full_idx);
%             end
%         end
%     end
% end
