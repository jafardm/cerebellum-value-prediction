function [amplifier_data,record_time,nofsample] = JDM_read_Intan_RHS2000_file_v1(path_to_raw)
...
rhs_files = dir(fullfile(path_to_raw, '*.rhs'));

% Ensure there's exactly one .rhs file
if isempty(rhs_files)
    error('No .rhs file found in the specified directory: %s', path_to_raw);
elseif length(rhs_files) > 1
    error('Multiple .rhs files found in the directory: %s. Please specify a unique file.', path_to_raw);
end

% Get the full file path
filename = fullfile(path_to_raw, rhs_files(1).name);
...
if (data_present)
    ...
    for i=1:num_data_blocks
        %t(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) =...
            fread(fid, num_samples_per_data_block, 'int32');
        if (num_amplifier_channels > 0)
            amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) =...
                fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
            if (dc_amp_data_saved ~= 0)
                dc_amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) =...
                    fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
            end
            %stim_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) =...
                fread(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16')';
        end
        if (num_board_adc_channels > 0)
            %board_adc_data(:, board_adc_index:(board_adc_index + num_samples_per_data_block - 1)) =...
                fread(fid, [num_samples_per_data_block, num_board_adc_channels], 'uint16')';
        end
        if (num_board_dac_channels > 0)
            %board_dac_data(:, board_dac_index:(board_dac_index + num_samples_per_data_block - 1)) =...
                fread(fid, [num_samples_per_data_block, num_board_dac_channels], 'uint16')';
        end
        if (num_board_dig_in_channels > 0)
            %board_dig_in_raw(board_dig_in_index:(board_dig_in_index + num_samples_per_data_block - 1)) =...
                fread(fid, num_samples_per_data_block, 'uint16');
        end
        if (num_board_dig_out_channels > 0)
            %board_dig_out_raw(board_dig_out_index:(board_dig_out_index + num_samples_per_data_block - 1)) =...
                fread(fid, num_samples_per_data_block, 'uint16');
        end
...
nofsample = size(amplifier_data,2);
