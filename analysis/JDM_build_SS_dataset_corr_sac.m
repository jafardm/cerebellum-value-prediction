function JDM_build_SS_dataset_corr_sac(data_path)
    JDM_params_funcs
    load(fullfile(data_path,'population_data','behave_data'));
    user = 'JDM';
    loc  = 'ctx';
    % reward conditions
    forced_conds = params.reward_tag4_conds;
    ...
    for counter_var = 1:numel(cell_var)
        ...
        data_out.(current_var).rate_tot_sac  = []; % [cell x 500 x 8 x n_conds]
        data_out.(current_var).vm_tot_sac    = []; % [cell x 500 x 8 x n_conds]
    ...
    for counter_sess = 1:num_sess
        ...
        for counter_clust = 1:num_clust
           ...
           if current_clust <= 0, continue; end
            ...
            ind_cs            = ismember(cell_types_clust,{'PC','CS'});
...
meta.ang_values_deg = params.sac.ang_values;  % bin centers
meta.animal_list    = animal_list;
meta.session_list   = session_list;
meta.sac_crit_ma    = data_behav.sess_cr_ma;  % per animal->session polygons
meta.sac_crit_ms    = data_behav.sess_cr_ms;
meta.date_built     = datestr(now, 'yyyy-mm-dd HH:MM:SS');
meta.params_version = 'JDM_params_funcs snapshot at build time';
...
for counter_var = 1:numel(cell_var)
    ...
    file_name = [current_var '_population_clique_corr_sac'];
...
% Save CS
data = data_out.CS;
file_name = 'CS_population_clique_corr_sac.mat';
...
function res = load_cs_data_tuned(current_path,current_sess,current_cell,...
 ...
 for counter_dir = 1:n_dirs  
    ...
    res.rate_tot = cat(4, cs_rate_tot_vis, cs_rate_tot_sac); % [500 x 8 x 4 x 2]
