...
data_info.sess_path = [path_data_monkey_data, session(1:7), filesep,...
    session, filesep];

data_info.session_name = regexprep(session(3:end),'-','');

% read session meta data and all rec meta data
sess_meta_data = readtable([data_info.sess_path ...
    data_info.session_name '.xls']);

recording_sessions = sess_meta_data.folder_name;
traial_num = sess_meta_data.num_trial;


ii=1;
loaded_data =load([data_info.sess_path recording_sessions{ii,:},filesep,'raw_data',...
       filesep,exp_name,'_',regexprep(recording_sessions{ii,:}(end-7:end),'-',''),'.mat']);
